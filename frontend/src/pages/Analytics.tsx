...
export default function Analytics({
  ...
  const calculateInsights = () => {
    ...
    // Insight 1: Content Creation (Always show if there's any data)
    const recentPosts = timeSeries.postsCreated && timeSeries.postsCreated.length > 0
      ? timeSeries.postsCreated.slice(-7).reduce((sum, item) => sum + item.count, 0)
      : 0
    const totalPosts = overview.totalPostsGenerated || 0
    
    if (recentPosts > 0) {
      const avgDaily = Math.round(recentPosts / 7)
      insights.push({
        title: 'Content Creation',
        value: `${avgDaily} ${avgDaily === 1 ? 'post' : 'posts'} per day`,
        description: avgDaily >= 5 
          ? "Excellent! You're creating content consistently"
          : avgDaily >= 3
          ? "You're creating content regularly this week"
          : "Keep up the momentum with more content",
        icon: <FileTextOutlined />,
        status: avgDaily >= 5 ? 'on-track' : avgDaily >= 2 ? 'needs-attention' : 'action-needed',
        priority: 1,
      })
    } else if (totalPosts > 0) {
      insights.push({
        title: 'Content Creation',
        value: `${totalPosts} ${totalPosts === 1 ? 'post' : 'posts'} created`,
        description: "Create more content this week to see daily trends",
        icon: <FileTextOutlined />,
        status: 'needs-attention',
        priority: 1,
      })
    } else {
      // Show even if no posts yet - encourage action
      insights.push({
        title: 'Content Creation',
        value: 'No posts yet',
        description: "Start creating your first post to see insights",
        icon: <FileTextOutlined />,
    ...
    // Insight 2: Publishing Completion (High priority)
    if (totalPosts > 0) {
      const published = overview.publishedPosts || 0
      const drafts = overview.draftPosts || 0
      const scheduled = overview.scheduledPosts || 0
      
      if (published === totalPosts && published > 0) {
        insights.push({
          title: 'Publishing',
          value: 'All posts published',
          description: "Nothing left in draft - great job!",
          icon: <CheckCircleOutlined />,
          status: 'on-track',
          priority: 2,
        })
      } else if (published > 0) {
        const unpublished = drafts + scheduled
        insights.push({
          title: 'Publishing',
          value: `${published} of ${totalPosts} published`,
          description: unpublished > 0 
            ? `${unpublished} ${unpublished === 1 ? 'post' : 'posts'} ${scheduled > 0 ? 'scheduled' : 'in draft'}`
            : "All created posts have been published",
          icon: <CheckCircleOutlined />,
          status: published >= totalPosts * 0.8 ? 'on-track' : published >= totalPosts * 0.5 ? 'needs-attention' : 'action-needed',
          priority: 2,
        })
      }
    }

    // Insight 3: AI-Assisted Content (Show if AI is being used)
    if (overview.totalAIGeneratedContent > 0) {
      const aiIdeas = overview.totalAIGeneratedContent
      const postsFromAI = totalPosts > 0 ? totalPosts : 0
      const efficiency = postsFromAI > 0 ? (postsFromAI / aiIdeas).toFixed(1) : '0'
      
      insights.push({
        title: 'AI-Assisted Content',
        value: `${aiIdeas} AI ${aiIdeas === 1 ? 'idea' : 'ideas'} → ${postsFromAI} ${postsFromAI === 1 ? 'post' : 'posts'}`,
        description: parseFloat(efficiency) >= 2
          ? "AI is helping you turn ideas into multiple posts efficiently"
          : parseFloat(efficiency) >= 1
          ? "AI is helping you create content from ideas"
          : "Try using AI ideas to create more posts",
        icon: <RobotOutlined />,
        status: parseFloat(efficiency) >= 2 ? 'on-track' : parseFloat(efficiency) >= 1 ? 'needs-attention' : 'action-needed',
        priority: 3,
      })
    }

    // Insight 4: Scheduling Strategy (Show calendar scheduling activity)
    const totalScheduled = overview.totalCalendarItems || 0
    const totalPublished = overview.publishedPosts || 0
    
    if (totalScheduled > 0 || totalPublished > 0) {
      if (totalScheduled >= totalPublished * 0.7 && totalPublished > 0) {
        insights.push({
          title: 'Scheduling Strategy',
          value: 'Most posts are scheduled ahead',
          description: totalScheduled === totalPublished 
            ? "All posts are planned in your calendar"
            : "You're planning most content ahead of time",
        ...
        insights.push({
          title: 'Scheduling Strategy',
          value: `${totalScheduled} ${totalScheduled === 1 ? 'post' : 'posts'} scheduled`,
          description: totalPublished > 0
            ? "Consider scheduling more posts ahead for better planning"
            : "Schedule posts in your calendar to plan ahead",
          icon: <CalendarOutlined />,
          status: totalScheduled >= 5 ? 'needs-attention' : 'action-needed',
          priority: 4,
        })
      }
    }
    ...
    if (recentChats > 0) {
      ...
      insights.push({
        ...
        icon: <MessageOutlined />,
        status: totalChats >= 10 ? 'needs-attention' : 'action-needed',
        priority: 7,
      })
    }
      ...
      <Layout className={styles.dashboardLayout}>
        ...
        <div id="key-insights" style={{ scrollMarginTop: '100px', paddingTop: '20px', marginTop: 48, marginBottom: 48 }}>
          <div style={{ 
            ...
            <Text type="secondary" style={{ fontSize: 14, marginTop: 8, display: 'block', color: '#8c8c8c' }}>
              Quick summary of how your content is being created, scheduled, and shared
            </Text>
          </div>
          {insights.length > 0 && (
              ...
              {insights.map((insight, index) => {
                ...
                const getBackgroundColor = () => {
                  switch (insight.status) {
                    case 'on-track':
                      return 'linear-gradient(135deg, #52c41a 0%, #73d13d 100%)' // Green
                    case 'needs-attention':
                      return 'linear-gradient(135deg, #faad14 0%, #ffc53d 100%)' // Yellow
                    case 'action-needed':
                      return 'linear-gradient(135deg, #ff4d4f 0%, #ff7875 100%)' // Red
                    default:
                      return 'linear-gradient(135deg, #1890ff 0%, #40a9ff 100%)' // Blue
                  }
                }
                ...
                const getStatusBadge = () => {
                  switch (insight.status) {
                    case 'on-track':
                      return { text: '🟢 On Track', color: '#52c41a' }
                    case 'needs-attention':
                      return { text: '🟡 Needs Attention', color: '#faad14' }
                    case 'action-needed':
                      return { text: '🔴 Action Needed', color: '#ff4d4f' }
                    default:
                      return { text: '', color: '#1890ff' }
                  }
                }
        ...
        <div id="time-series" style={{ scrollMarginTop: '100px', paddingTop: '20px', marginTop: 48, marginBottom: 48 }}>
          ...
          <Row gutter={[20, 20]}>
              ...
              <Card 
                ...
                title={
                  <Space>
                    ...
                    <span style={{ fontWeight: 600 }}>Content Creation Over Time</span>
                  </Space>
                }
              >
                {timeSeries.postsCreated && timeSeries.postsCreated.length > 0 ? (
                  <ResponsiveContainer width="100%" height={300}>
                    <LineChart data={prepareDailyData(timeSeries.postsCreated)}>
                      <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" />
                      <XAxis dataKey="date" tick={{ fontSize: 11, fill: '#666' }} stroke="#d9d9d9" />
                      <YAxis 
                        tick={{ fontSize: 11, fill: '#666' }}
                        label={{ value: 'Posts per day', angle: -90, position: 'insideLeft', style: { fontSize: 12, fill: '#666' } }}
                        stroke="#d9d9d9"
                        allowDecimals={false}
                      />
                      <Tooltip 
                        contentStyle={{ borderRadius: 8, border: '1px solid #e8e8e8' }}
                        formatter={(value: number | undefined) => [value || 0, 'Posts created']}
            ...
            <Col xs={24} lg={12}>
              ...
              >
                {timeSeries.publishedPosts && timeSeries.scheduledPosts && 
                 (timeSeries.publishedPosts.length > 0 || timeSeries.scheduledPosts.length > 0) ? (
                  <ResponsiveContainer width="100%" height={300}>
                    <LineChart data={mergeTimeSeries(
                      ...
                      <Tooltip 
                        ...
                        formatter={(value: number | undefined, name: string | undefined) => {
                          const label = name === 'published' ? 'Published' : 'Scheduled'
                      ...
                      <Legend 
                        formatter={(value) => {
                          if (value === 'published') return 'Published posts/day'
                          if (value === 'scheduled') return 'Scheduled posts/day'
                          return value
                        }}
                      />
                      <Line
              ...
              <Card 
                ...
                title={
                  <Space>
                    ...
                    <span style={{ fontWeight: 600 }}>AI Generation Activity</span>
                  </Space>
                }
              >
                {timeSeries.aiContent && timeSeries.aiContent.length > 0 ? (
                  <ResponsiveContainer width="100%" height={300}>
                    <LineChart data={prepareDailyData(timeSeries.aiContent)}>
                      <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" />
                      <XAxis dataKey="date" tick={{ fontSize: 11, fill: '#666' }} stroke="#d9d9d9" />
                      <YAxis 
                        tick={{ fontSize: 11, fill: '#666' }}
                        label={{ value: 'AI pieces per day', angle: -90, position: 'insideLeft', style: { fontSize: 12, fill: '#666' } }}
                        stroke="#d9d9d9"
                        allowDecimals={false}
                      />
                      <Tooltip 
                        contentStyle={{ borderRadius: 8, border: '1px solid #e8e8e8' }}
                        formatter={(value: number | undefined) => [value || 0, 'AI pieces generated']}
                        labelFormatter={(label) => `Date: ${label}`}
                      />
                      <Line
                        type="monotone"
            ...
            {/* Trend 4: Publishing Method Trend (Very Important) */}
            <Col xs={24} lg={12}>
              <Card 
                hoverable
                style={{ 
                ...
                title={
                  <Space>
                    ...
                    <span style={{ fontWeight: 600 }}>Publishing Method Trend</span>
                  </Space>
                }
              >
                {timeSeries.calendarPublishes && timeSeries.directPublishes && 
                 (timeSeries.calendarPublishes.length > 0 || timeSeries.directPublishes.length > 0) ? (
                  <ResponsiveContainer width="100%" height={300}>
                    <LineChart data={mergeTimeSeries(
                      ...
                      <Tooltip 
                        contentStyle={{ borderRadius: 8, border: '1px solid #e8e8e8' }}
                        formatter={(value: number | undefined, name: string | undefined) => {
                          const label = name === 'calendar' ? 'Calendar publishes' : 'Direct publishes'
                          return [value || 0, label]
                        }}
                        labelFormatter={(label) => `Date: ${label}`}
                      />
                      <Legend 
                        formatter={(value) => {
                          if (value === 'calendar') return 'Calendar publishes/day'
                          if (value === 'direct') return 'Direct publishes/day'
                          return value
                        }}
                      />
                      <Line
              ...
              <Card 
                ...
                title={
                  <Space>
                    ...
                    <span style={{ fontWeight: 600 }}>User Interaction Activity</span>
                  </Space>
                }
              >
                {timeSeries.conversations && timeSeries.events && 
                 (timeSeries.conversations.length > 0 || timeSeries.events.length > 0) ? (
                  <ResponsiveContainer width="100%" height={300}>
                    <LineChart data={mergeTimeSeries(
                      timeSeries.conversations || [],
                      timeSeries.events || [],
                      'chatSessions',
                      'events'
                    )}>
                      ...
                      <YAxis 
                        tick={{ fontSize: 11, fill: '#666' }}
                        label={{ value: 'Activity per day', angle: -90, position: 'insideLeft', style: { fontSize: 12, fill: '#666' } }}
                        stroke="#d9d9d9"
                        allowDecimals={false}
                      />
                      <Tooltip 
                        contentStyle={{ borderRadius: 8, border: '1px solid #e8e8e8' }}
                        formatter={(value: number | undefined, name: string | undefined) => {
                          const label = name === 'chatSessions' ? 'AI chat sessions' : 'Events created'
                          return [value || 0, label]
                        }}
                        labelFormatter={(label) => `Date: ${label}`}
                      />
                      <Legend 
                        formatter={(value) => {
                          if (value === 'chatSessions') return 'AI chat sessions/day'
                          if (value === 'events') return 'Events created/day'
                          return value
                        }}
                      />
                      <Line
        ...
        <div id="breakdown" style={{ scrollMarginTop: '100px', paddingTop: '20px', marginTop: 48, marginBottom: 48 }}>
          ...
          <div style={{ marginBottom: 32 }}>
            ...
            <Row gutter={[16, 16]}>
              {(() => {
                // Always include Instagram even if no posts
                const platformsWithData = { ...breakdown.postsByPlatform }
                if (!platformsWithData['instagram']) {
                  platformsWithData['instagram'] = 0
                }
                
                // Define all platforms to show (always include these)
                const allPlatforms = ['linkedin', 'twitter', 'facebook', 'instagram', 'sms', 'whatsapp']
                
                // Create entries for all platforms, using actual data or 0
                const platformEntries = allPlatforms.map(platform => [
                  platform,
                  platformsWithData[platform] || 0
                ])
                
                return platformEntries
                  .sort(([, a], [, b]) => (b as number) - (a as number))
                  .map(([platform, count]) => {
                    const totalPlatformPosts = Object.values(platformsWithData).reduce((sum: number, c: any) => sum + (c || 0), 0)
                    const percentage = totalPlatformPosts > 0 ? Math.round(((count as number) / totalPlatformPosts) * 100) : 0
                    ...
                    const displayName = platformDisplayNames[normalizedPlatform] || String(platform).charAt(0).toUpperCase() + String(platform).slice(1)
          ...
          {breakdown.calendarItemsByPlatform && Object.keys(breakdown.calendarItemsByPlatform).length > 0 && (
            <div style={{ marginBottom: 32 }}>
              <Title level={4} style={{ marginBottom: 16 }}>Total Generated Events by Platform</Title>
              <Row gutter={[16, 16]}>
                {Object.entries(breakdown.calendarItemsByPlatform)
                  ...
                  .map(([platform, count]) => {
                    ...
                    const displayName = platformDisplayNames[String(platform).toLowerCase()] || String(platform).charAt(0).toUpperCase() + String(platform).slice(1)
                      ...
                      <Col xs={24} sm={12} md={8} lg={6} xl={6} key={`calendar-platform-${platform}`}>
                        ...
                        >
                          <div style={{ marginBottom: 12 }}>
                            <Text strong style={{ fontSize: 15, color: 'rgba(255,255,255,0.9)' }}>
                              {displayName}
                            </Text>
          ...
          {/* Direct Posts by Platform */}
          {breakdown.postsByPlatform && Object.keys(breakdown.postsByPlatform).length > 0 && (
            <div style={{ marginBottom: 32 }}>
              <Title level={4} style={{ marginBottom: 16 }}>Direct Posts by Platform</Title>
              <Text type="secondary" style={{ display: 'block', marginBottom: 16, fontSize: 14 }}>
                Posts published directly from social dashboard (not from calendar items)
              </Text>
              <Row gutter={[16, 16]}>
                {Object.entries(breakdown.postsByPlatform)
                  .map(([platform, count]) => {
                    // Calculate direct posts: total posts - calendar items for this platform
                    // Note: This assumes calendarItemsByPlatform represents published calendar items
                    const calendarCount = breakdown.calendarItemsByPlatform?.[platform] || 0
                    const directCount = Math.max(0, (count as number) - calendarCount)
                    
                    return { platform, directCount, totalCount: count as number }
                  })
                  .filter(item => item.directCount > 0) // Only show platforms with direct posts
                  .sort((a, b) => b.directCount - a.directCount)
                  .map(({ platform, directCount }) => {
                    
                    // Calculate total direct posts across all platforms
                    const totalDirect = Object.entries(breakdown.postsByPlatform).reduce((sum, [p, c]) => {
                      const calCount = breakdown.calendarItemsByPlatform?.[p] || 0
                      return sum + Math.max(0, (c as number) - calCount)
                    }, 0)
                    const percentage = totalDirect > 0 ? Math.round((directCount / totalDirect) * 100) : 0
                    ...
                    return (
                      <Col xs={24} sm={12} md={8} lg={6} xl={6} key={`direct-platform-${platform}`}>
                        <Card 
                          hoverable
                          style={{ 
                            borderRadius: 16,
                ...
                <AntdTooltip 
                  title={
                    <div style={{ maxWidth: 400 }}>
                      ...
                      Ideation = Total conversations (melo.conversations)<br />
                      Planning = Calendar items with status 'draft' (melo.calendaritems)<br />
                      Live = Published posts (melo.socialmediaposts with status 'published')<br />
                      Conversion Rate = (Live / Ideation) × 100
                    </div>
                  }
                  placement="right"
                >
              ...
              <div id="ai-efficiency-roi" style={{ scrollMarginTop: '100px', marginBottom: 48 }}>
                <div style={{ display: 'flex', alignItems: 'center', marginBottom: 24 }}>
                  ...
                  <AntdTooltip 
                    title={
                      <div style={{ maxWidth: 400 }}>
                        <strong>Calculation:</strong><br />
                        Processing time from melo.aigeneratedcontents (processingTime in milliseconds)<br />
                        Human benchmark = 1800 seconds (30 minutes) per post<br />
                        Time Saved = (30 min × Total AI Posts) - Sum(processingTime)<br />
                        Efficiency Ratio = Human Time Total / AI Processing Time Total
                      </div>
                    }
                    placement="right"
                  >
                ...
                <Row gutter={[20, 20]}>
                  ...
                  <Col xs={24} sm={12} md={8}>
                    ...
                    >
                      <Text style={{ fontSize: 13, opacity: 0.9, display: 'block', marginBottom: 8, color: '#fff' }}>
                        EFFICIENCY RATIO
                      </Text>
                      <Title level={1} style={{ color: '#fff', margin: 0, fontSize: 48, fontWeight: 700 }}>
                        {analytics.advancedAnalytics.aiEfficiency.efficiencyRatio}x
                      </Title>
                      <Text style={{ fontSize: 14, opacity: 0.9, display: 'block', marginTop: 4, color: '#fff' }}>
                  ...
                  <Col xs={24} sm={12} md={8}>
                    ...
                    >
                      ...
                      <Title level={1} style={{ color: '#fff', margin: 0, fontSize: 48, fontWeight: 700 }}>
                        {Math.round(analytics.advancedAnalytics.aiEfficiency.avgProcessingTime / 1000)} sec
                      </Title>
                      <Text style={{ fontSize: 14, opacity: 0.9, display: 'block', marginTop: 4, color: '#fff' }}>
            ...
            <div id="cross-platform-strategy" style={{ scrollMarginTop: '100px', marginBottom: 48 }}>
              <div style={{ display: 'flex', alignItems: 'center', marginBottom: 24 }}>
                ...
                <AntdTooltip 
                  title={
                    <div style={{ maxWidth: 400 }}>
                      <strong>Calculation:</strong><br />
                      Planned = Count of calendar items by platform (melo.calendaritems grouped by platform)<br />
                      Published = Count of published posts by platform (melo.socialmediaposts with status 'published' grouped by platform)<br />
                      Gap = Planned - Published<br />
                      Completion Rate = (Published / Planned) × 100
                    </div>
                  }
                  placement="right"
                >
              ...
              <Card style={{ borderRadius: 16, border: 'none', boxShadow: '0 4px 12px rgba(0,0,0,0.08)' }}>
                <ResponsiveContainer width="100%" height={400}>
                  <ComposedChart 
                    data={(() => {
                      const data = analytics.advancedAnalytics.crossPlatformStrategy || []
                      // Debug: Log the data structure
                      console.log('[Cross-Platform Strategy] Chart data:', data)
                  ...
                  >
                    ...
                    <Tooltip 
                      contentStyle={{ borderRadius: 8, border: '1px solid #e8e8e8' }}
                      formatter={(value: number | undefined, name: string | undefined) => {
                        const label = name === 'planned' ? 'Planned' : name === 'published' ? 'Published' : name || ''
                        return [value || 0, label]
                      }}
                    />
                    <Legend 
                ...
                <div style={{ display: 'flex', alignItems: 'center', marginBottom: 24 }}>
                  ...
                  <AntdTooltip 
                    title={
                      <div style={{ maxWidth: 400 }}>
                        <strong>Calculation:</strong><br />
                        Total Assets = Count of all media files (melo.mediafiles)<br />
                        Used Assets = Count of posts with non-empty mediaAttachments array (melo.socialmediaposts where mediaAttachments[0].url exists)<br />
                        Utilization Rate = (Used Assets / Total Assets) × 100<br />
                        Untapped Potential = Total Assets - Used Assets
                      </div>
                    }
                    placement="right"
                  >
              ...
              <div style={{ display: 'flex', alignItems: 'center', marginBottom: 24 }}>
                ...
                <AntdTooltip 
                  title={
                    <div style={{ maxWidth: 400 }}>
                      <strong>Calculation:</strong><br />
                      Hashtags extracted from content fields in melo.calendaritems and melo.socialmediaposts using regex pattern /#(\w+)/g<br />
                      Frequency counted per hashtag (case-insensitive)<br />
                      Top 20 hashtags displayed by frequency<br />
                      Tag size proportional to usage count
                    </div>
                  }
                  placement="right"
                >
