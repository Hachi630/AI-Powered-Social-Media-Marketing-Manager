...
// Use absolute URL path for demo image to ensure it works in all environments
// This resolves the image display issue in demo mode deployment
export const getDemoImageUrl = (): string => {
  // In browser environment, use origin; otherwise use relative path
  if (typeof window !== 'undefined' && window.location?.origin) {
    return `${window.location.origin}/img/i2.jpg`;
  }
  return '/img/i2.jpg';
};
...
// Function to create calendar items with dynamic image URL
export const createDemoCalendarItems = (): CalendarItem[] => {
...
// Keep backward compatibility - lazy initialize on first access
export const DEMO_CALENDAR_ITEMS: CalendarItem[] = createDemoCalendarItems();
