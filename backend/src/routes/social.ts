...
router.get(
  ...
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      oauthStates.set(
        state,
        JSON.stringify({ userId: user._id.toString(), type: "facebook_only" })
      );
...
router.get(
  ...
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      oauthStates.set(
        state,
        JSON.stringify({ userId: user._id.toString(), type: "instagram" })
      );
...
router.get("/instagram/callback", async (req: Request, res: Response) => {
  try {
    ...
    if (connectionType === "facebook_only") {
      console.log(
        "[Instagram OAuth Callback] Facebook-only connection requested, skipping Instagram detection"
      );

      if (pages.length > 0) {
        const firstPage = pages[0];
        console.log(
          "[Instagram OAuth Callback] Saving Facebook-only connection for page:",
          firstPage.name
        );
        ...
        try {
          ...
          if (user.socialConnections.instagram) {
            console.log(
              "[Instagram OAuth Callback] Removing existing Instagram connection for Facebook-only connection"
            );
          ...
          if (verifyUser?.socialConnections?.instagram) {
            console.error(
              "[Instagram OAuth Callback] WARNING: Instagram connection still exists after deletion! Force removing..."
            );
            delete verifyUser.socialConnections.instagram;
            await verifyUser.save();
            console.log(
              "[Instagram OAuth Callback] Instagram connection force removed and verified"
            );
          } else {
            console.log(
              "[Instagram OAuth Callback] Verified: Instagram connection successfully removed"
            );
          }

          console.log(
            "[Instagram OAuth Callback] Saved Facebook-only connection:",
            {
              userId: user._id,
          ...
          console.error(
            "[Instagram OAuth Callback] Failed to save Facebook-only connection:",
        ...
        if (isFrontendCallback) {
          ...
          return res.redirect(
            `${process.env.FRONTEND_URL || "http://localhost:3000"}/auth/instagram/callback?error=no_pages`
          );
        }
      }
    }
    ...
    if (pagesWithInstagram.length === 0) {
      ...
      if (pages.length > 0) {
        ...
        try {
          console.log(
            "[Instagram OAuth Callback] Attempting direct connection method..."
          );
          ...
          console.log(
            "[Instagram OAuth Callback] Token type:",
            firstPage.accessToken ? "page token" : "user token"
          );
          console.log(
            "[Instagram OAuth Callback] Token preview:",
          ...
          console.log(
            "[Instagram OAuth Callback] ✅ Successfully found Instagram account via direct method:",
            {
          ...
          console.log(
            "[Instagram OAuth Callback] Successfully connected via direct method:",
            {
              userId: user._id,
          ...
          console.log(
            "[Instagram OAuth Callback] Attempting fallback: Save connection without Instagram detection"
          );
          ...
          try {
            ...
            console.log(
              "[Instagram OAuth Callback] Saved connection with fallback method:",
              {
                userId: user._id,
                facebookPageId: firstPage.id,
            ...
            if (req.headers.authorization) {
              ...
              res.json({
                success: true,
                message:
                  "Successfully connected (Instagram account will be resolved when posting)",
                redirectUrl, // Redirect to social dashboard like Twitter
                instagram: {
            ...
            console.error(
              "[Instagram OAuth Callback] Fallback method also failed:",
      ...
      if (pages.length > 0) {
        const firstPage = pages[0];
        console.log(
          "[Instagram OAuth Callback] All Instagram detection methods failed, but allowing Facebook-only connection"
        );
        ...
        try {
          ...
          if (user.socialConnections.instagram) {
            console.log(
              "[Instagram OAuth Callback] Removing existing Instagram connection (fallback Facebook-only connection)"
            );
            delete user.socialConnections.instagram;
          }
          ...
          if (verifyUser2?.socialConnections?.instagram) {
            console.error(
              "[Instagram OAuth Callback] WARNING: Instagram connection still exists in fallback method! Force removing..."
            );
          ...
          if (isFrontendCallback) {
            return res.json({
              success: true,
              message:
                "Successfully connected Facebook Page. Note: Instagram sharing requires a Business/Creator account connected to your Facebook Page.",
              redirectUrl, // Redirect to social dashboard like Twitter
              facebook: {
                pageId: firstPage.id,
                pageName: pageNameResponse.data.name,
              ...
              instagram: null,
              warning:
                "Instagram not connected. To use Instagram sharing, please connect an Instagram Business/Creator account to your Facebook Page.",
            });
          } else {
            return res.redirect(redirectUrl);
          }
          ...
          console.error(
            "[Instagram OAuth Callback] Failed to save Facebook connection:",
            saveError.response?.data || saveError.message
          );
          // Fall through to error
        }
    ...
    if (pagesWithInstagram.length === 1) {
      ...
      console.log(
        "[Instagram OAuth Callback] Redirecting to Facebook Page:",
        facebookPageId
      );
      res.redirect(`https://www.facebook.com/pages/manage/${facebookPageId}`);
    } else {
    ...
    console.error(
      "[Instagram OAuth Callback] Error response:",
      error.response?.data
    );
    const errorMessage =
      error.response?.data?.error?.message || error.message || "oauth_failed";
    console.error(
      "[Instagram OAuth Callback] Redirecting to settings with error:",
      errorMessage
    );
    res.redirect(
...
router.get(
  ...
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      const isInstagramUserIdSameAsFacebook =
        instagram?.userId &&
        facebook?.userId &&
        instagram.userId === facebook.userId;
      ...
      if (
        ...
        !instagram.accessToken ||
        !instagram.userId ||
        isInstagramUserIdSameAsFacebook
      ) {
        ...
        if (isInstagramUserIdSameAsFacebook) {
          console.log(
            "[Instagram Status] Instagram userId matches Facebook userId - this is not a real Instagram connection"
          );
        }
        return res.json({
          success: true,
      ...
      try {
        ...
        // Instagram Graph API endpoint for getting user info
        const profileResponse = await axios.get(
          `https://graph.facebook.com/v18.0/${instagram.userId}?fields=username,account_type&access_token=${instagram.accessToken}`
        );

        // Try to get more info from Facebook Page (since Instagram is linked to a Facebook Page)
        let name = null;
        let picture = null;
        try {
          // Get the Facebook Page info that owns this Instagram account
          const facebook = freshUser.socialConnections?.facebook;
          if (facebook?.accessToken) {
            const pagesResponse = await axios.get(
              `https://graph.facebook.com/v18.0/me/accounts?access_token=${facebook.accessToken}`
            );
            // Find the page that owns this Instagram account
          ...
          console.warn(
            "[Instagram Status] Failed to get page info:",
        ...
        profile = {
          ...
          name: name || profileResponse.data.username || instagram.username,
          email: null, // Instagram API doesn't provide email
          picture: picture,
          accountType:
            profileResponse.data.account_type || instagram.accountType,
        };
    ...
    try {
      ...
      console.log("[Facebook Share] Request received:", {
        ...
        instagramToken: freshUser.socialConnections?.instagram?.accessToken
          ? "exists"
          : "missing",
        facebookTokenPreview: freshUser.socialConnections?.facebook?.accessToken
          ? freshUser.socialConnections.facebook.accessToken.substring(0, 20) +
            "..."
          : "N/A",
        facebookUserId: freshUser.socialConnections?.facebook?.userId || "N/A",
        instagramUserId:
          freshUser.socialConnections?.instagram?.userId || "N/A",
      });
          ...
          try {
            ...
            if (pagesResponse.data.data && pagesResponse.data.data.length > 0) {
              ...
              if (!freshUser.socialConnections.facebook) {
                freshUser.socialConnections.facebook = {
                  accessToken: facebookToken,
                };
              }
              freshUser.socialConnections.facebook.userId = facebookUserId;
              freshUser.socialConnections.facebook.expiresAt =
                instagram.expiresAt;
      ...
      if (facebookUserId && facebookToken) {
        try {
          console.log(
            "[Facebook Share] Getting Page access token for:",
            facebookUserId
          );
          const pagesResponse = await axios.get(
            `https://graph.facebook.com/v18.0/me/accounts?access_token=${facebookToken}`
          ...
          if (pagesResponse.data.data && pagesResponse.data.data.length > 0) {
            ...
            const targetPage = pagesResponse.data.data.find(
              (page: any) => page.id === facebookUserId
            );
            if (targetPage && targetPage.access_token) {
              pageAccessToken = targetPage.access_token;
              ...
              console.log(
                "[Facebook Share] Page not found in accounts, using user token"
              );
          ...
          console.error(
            "[Facebook Share] Error getting Page access token:",
            error.response?.data || error.message
          );
      ...
      if (!pageAccessToken || !facebookUserId) {
        return res.status(400).json({
          success: false,
          message:
            "Facebook account not connected. Please connect your Facebook or Instagram account first.",
          requiresAuth: true,
        });
      }
    ...
    try {
      ...
      const debugInfo = {
        ...
        facebook: {
          ...
          tokenPreview: facebook?.accessToken
            ? facebook.accessToken.substring(0, 20) + "..."
            : "N/A",
          userId: facebook?.userId || "N/A",
          expiresAt: facebook?.expiresAt || "N/A",
          isExpired: facebook?.expiresAt
            ? new Date() > facebook.expiresAt
            : "N/A",
        },
        instagram: {
          exists: !!instagram,
          hasAccessToken: !!instagram?.accessToken,
          hasUserId: !!instagram?.userId,
          tokenPreview: instagram?.accessToken
            ? instagram.accessToken.substring(0, 20) + "..."
            : "N/A",
          userId: instagram?.userId || "N/A",
          username: instagram?.username || "N/A",
          expiresAt: instagram?.expiresAt || "N/A",
          isExpired: instagram?.expiresAt
            ? new Date() > instagram.expiresAt
            : "N/A",
        },
        envCheck: {
      ...
      try {
        const axios = require("axios");
        const profileResponse = await axios.get(
          `https://graph.facebook.com/v18.0/me?fields=id,name,email,picture&access_token=${facebook.accessToken}`
        );
        profile = {
          id: profileResponse.data.id,
          name: profileResponse.data.name || null,
          email: profileResponse.data.email || null,
          picture: profileResponse.data.picture?.data?.url || null,
        };
      } catch (profileError: any) {
        console.warn(
          "[Facebook Status] Failed to get user profile:",
          profileError.response?.data || profileError.message
        );
        // Still return connected status even if we can't get profile
        profile = {
        ...
        if (verifyUser?.socialConnections?.facebook) {
          console.error(
            "[Facebook Disconnect] WARNING: Facebook connection still exists after $unset!",
            {
              ...
              hasAccessToken:
                !!verifyUser.socialConnections.facebook.accessToken,
            }
          );
          console.error(
            "[Facebook Disconnect] Force removing with direct delete..."
          );
          ...
          if (doubleVerify?.socialConnections?.facebook) {
            console.error(
              "[Facebook Disconnect] CRITICAL: Facebook connection STILL exists after double removal!"
            );
            // Last resort: set to undefined explicitly
            doubleVerify.socialConnections.facebook = undefined as any;
            await doubleVerify.save();
            console.log(
              "[Facebook Disconnect] Set to undefined as last resort"
            );
          } else {
            console.log(
              "[Facebook Disconnect] Facebook connection force removed and double verified"
            );
          }
        } else {
          console.log(
        ...
        if (verifyUser?.socialConnections?.instagram) {
          console.error(
            "[Instagram Disconnect] WARNING: Instagram connection still exists after $unset!",
            {
              ...
              hasAccessToken:
                !!verifyUser.socialConnections.instagram.accessToken,
            }
          );
          console.error(
            "[Instagram Disconnect] Force removing with direct delete..."
          );
          // Try direct delete as fallback
          delete verifyUser.socialConnections.instagram;
          await verifyUser.save();
          ...
          if (doubleVerify?.socialConnections?.instagram) {
            console.error(
              "[Instagram Disconnect] CRITICAL: Instagram connection STILL exists after double removal!"
            );
            // Last resort: set to undefined explicitly
            doubleVerify.socialConnections.instagram = undefined as any;
            await doubleVerify.save();
            console.log(
              "[Instagram Disconnect] Set to undefined as last resort"
            );
          } else {
            console.log(
              "[Instagram Disconnect] Instagram connection force removed and double verified"
            );
          }
        } else {
          console.log(
