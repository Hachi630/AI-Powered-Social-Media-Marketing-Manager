...
const FACEBOOK_REDIRECT_URI =
  ...
  (process.env.FRONTEND_URL
    ? `${process.env.FRONTEND_URL}/auth/facebook/callback`
    : "http://localhost:3000/auth/facebook/callback");

/**
 * Generate Facebook OAuth authorization URL (for Facebook Page sharing only)
 * @param state - OAuth state parameter
 * @param includeBusinessManagement - Whether to include business_management scope (required for Instagram and some Business Account features)
 */
export function getFacebookAuthUrl(
  ...
  // business_management scope is needed for:
  // - Instagram Business Account access
  // - Some Business Account features
  // - Better access to Facebook Pages managed by Business Accounts
  const scopes = [
    ...
    // business_management is needed for Instagram and Business Account features
...
export async function exchangeCodeForToken(code: string): Promise<{
  ...
  try {
    ...
    console.error(
      "[Facebook Service] Error exchanging code for token:",
...
export async function getLongLivedToken(shortLivedToken: string): Promise<{
  ...
  try {
    ...
    console.error(
      "[Facebook Service] Error getting long-lived token:",
      error.response?.data || error.message
    );
    throw new Error(
...
> {
  try {
    ...
    if (!pagesResponse.data.data || pagesResponse.data.data.length === 0) {
      console.log(
        "[Facebook Service] No pages found. User may not have any Facebook Pages."
      );
      return [];
    }

    console.log(
      "[Facebook Service] Found",
    ...
    console.error(
      "[Facebook Service] Error getting Facebook pages:",
      error.response?.data || error.message
    );
    throw new Error(
...
export async function shareToFacebook(
  ...
  try {
    ...
    // If image is provided, create a post with photo
      ...
      if (content.imagePath && fs.existsSync(content.imagePath)) {
        ...
        const photoResponse = await axios.post<{
          id: string;
          post_id?: string;
        }>(`https://graph.facebook.com/v24.0/${pageId}/photos`, formData, {
          headers: formData.getHeaders(),
        });
    ...
    // Ensure postId is defined (TypeScript safety check)
    if (!postId) {
      throw new Error("Failed to create post: postId is undefined");
    }
    ...
    console.error(
      "[Facebook Service] Error sharing to Facebook:",
      error.response?.data || error.message
    );
    throw new Error(
      `Failed to share to Facebook: ${error.response?.data?.error?.message || error.message}`
    );
  }
}
