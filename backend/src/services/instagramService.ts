...
const INSTAGRAM_REDIRECT_URI =
  ...
  (process.env.FRONTEND_URL
    ? `${process.env.FRONTEND_URL}/auth/instagram/callback`
...
export function getInstagramAuthUrl(
  state: string,
  includeBusinessManagement: boolean = true
): string {
  if (!FACEBOOK_APP_ID) {
 ...
 */
export async function getFacebookPagesWithInstagram(
  accessToken: string
): Promise<
  Array<{
    id: string;
...
> {
  try {
    console.log(
      "[Instagram Service] Requesting Facebook Pages with Instagram info..."
    );
    ...
    if (!pagesResponse.data.data || pagesResponse.data.data.length === 0) {
      console.log(
        "[Instagram Service] No pages found. User may not have any Facebook Pages."
      );
      return [];
    }

    console.log(
      "[Instagram Service] Found",
      pagesResponse.data.data.length,
      "pages"
    );
    ...
    const pagesWithInstagram = (
      await Promise.all(
        pagesResponse.data.data.map(async (page: any) => {
          try {
            console.log(
              `[Instagram Service] Checking Instagram for page: ${page.name} (${page.id})`
            );

            // Try to get Instagram Business Account using page access token
            // IMPORTANT: Use page access token, not user access token
            // Also try with user access token as fallback
            let instagramResponse: any = null;

            // Method 1: Try with page access token first (most reliable)
            try {
              instagramResponse = await axios.get<{
              ...
              console.log(
                `[Instagram Service] Method 1 (page token) response for ${page.name}:`,
                {
                  hasInstagram:
              ...
              console.log(
                `[Instagram Service] Method 1 (page token) failed for ${page.name}:`,
                pageTokenError.response?.data || pageTokenError.message
              );

              // Method 1b: Try with user access token as fallback
              try {
                instagramResponse = await axios.get<{
                  instagram_business_account?: {
                    id: string;
                ...
                console.log(
                  `[Instagram Service] Method 1b (user token) response for ${page.name}:`,
                  {
                    hasInstagram:
                      !!instagramResponse.data.instagram_business_account,
                    error: instagramResponse.data.error,
                ...
                console.log(
                  `[Instagram Service] Method 1b (user token) also failed for ${page.name}:`,
                  userTokenError.response?.data || userTokenError.message
                );
            ...
            if (!instagramResponse) {
              console.log(
                `[Instagram Service] No response from Instagram API for ${page.name}, trying alternative methods...`
              );
              // Fall through to alternative methods below
            } else {
              console.log(
                `[Instagram Service] Instagram response for ${page.name}:`,
                {
                  hasInstagramAccount:
                    !!instagramResponse.data.instagram_business_account,
                  instagramAccountId:
              ...
              if (instagramResponse.data.instagram_business_account) {
                const instagramAccountId =
                  instagramResponse.data.instagram_business_account.id;
                console.log(
                  `[Instagram Service] Found Instagram account: ${instagramAccountId} for page: ${page.name}`
                );
                ...
                try {
                  ...
                  console.log(
                    `[Instagram Service] Instagram account details:`,
                  ...
                  console.log(
                    `[Instagram Service] Failed to get account_type, trying username only:`,
                ...
                console.log(
                  `[Instagram Service] No Instagram account found for page: ${page.name}`
                );
                ...
                try {
                  ...
                  console.log(`[Instagram Service] Method 1 - Nested fields:`, {
                    hasInstagram:
                      !!pageDetailsResponse.data.instagram_business_account,
                    instagram:
                      pageDetailsResponse.data.instagram_business_account,
                  ...
                  if (pageDetailsResponse.data.instagram_business_account) {
                    const instagramAccountId =
                      pageDetailsResponse.data.instagram_business_account.id;
                  ...
                  try {
                    ...
                    console.log(
                      `[Instagram Service] Method 2a - Business info:`,
                    ...
                    if (businessResponse.data.business) {
                      ...
                      console.log(
                        `[Instagram Service] Method 2b - Instagram accounts:`,
                        instagramAccountsResponse.data
                      );

                      if (
                        instagramAccountsResponse.data.data &&
                        instagramAccountsResponse.data.data.length > 0
                      ) {
                        const instagramAccount =
                          instagramAccountsResponse.data.data[0];
                        ...
                        try {
                          ...
                          instagramUsername =
                            accountDetailsResponse.data.username;
                        } catch (accountError: any) {
                          // If account_type field is not available, try without it
                          const usernameResponse = await axios.get<{
                            username: string;
                    ...
                    console.log(
                      `[Instagram Service] Method 2 failed:`,
                      m2Error.response?.data || m2Error.message
                    );
                  }
                  ...
                  try {
                    ...
                    console.log(
                      `[Instagram Service] Method 3 - Using user token:`,
                      {
                        hasInstagram:
                          !!userTokenResponse.data.instagram_business_account,
                        instagram:
                          userTokenResponse.data.instagram_business_account,
                      }
                    );
                    ...
                    if (userTokenResponse.data.instagram_business_account) {
                      const instagramAccountId =
                        userTokenResponse.data.instagram_business_account.id;
                      // Try to get username (account_type may not be available)
                      let instagramUsername: string;
                      try {
                        const accountDetailsResponse = await axios.get<{
                    ...
                    console.log(
                      `[Instagram Service] Method 3 failed:`,
                      m3Error.response?.data || m3Error.message
                    );
                  }
                } catch (altError: any) {
                  console.log(
                    `[Instagram Service] Alternative methods failed:`,
                ...
                console.log(
                  `[Instagram Service] Returning page without Instagram detection: ${page.name}`
                );
                return {
                  id: page.id,
                  name: page.name,
            ...
            console.error(
              `[Instagram Service] Error checking Instagram for page ${page.name}:`,
              {
                error: error.message,
    ...
    console.error(
      "[Instagram Service] Error getting Facebook pages:",
      error.response?.data || error.message
    );
    throw new Error(
...
export async function getInstagramAccountIdForPage(
  ...
  try {
    console.log(
      `[getInstagramAccountIdForPage] Getting Instagram for page: ${pageId}`
    );

    // Method 1: Try standard field query
    let pageResponse: {
      data: { name?: string; instagram_business_account?: { id: string } };
    } | null = null;
    try {
      ...
      console.log(
        `[getInstagramAccountIdForPage] Method 1 failed:`,
        error.response?.data || error.message
      );
      pageResponse = null;
    }
    ...
    if (!pageResponse?.data?.instagram_business_account) {
      try {
        ...
        console.log(
          `[getInstagramAccountIdForPage] Method 3a - Business:`,
          businessResponse.data
        );
        ...
        if (businessResponse.data.business) {
          const businessId = businessResponse.data.business.id;
          console.log(
            `[getInstagramAccountIdForPage] Found business ID: ${businessId}`
          );
          ...
          try {
            ...
            console.log(
              `[getInstagramAccountIdForPage] Method 3b - Instagram accounts:`,
              instagramAccountsResponse.data
            );
            ...
            if (
              ...
              try {
                ...
                instagramUsername = accountDetailsResponse.data.username;
                accountType =
                  accountDetailsResponse.data.account_type === "BUSINESS"
                    ? "BUSINESS"
                    : "CREATOR";
              } catch (accountError: any) {
                // If account_type field is not available, try without it
            ...
            console.log(
              `[getInstagramAccountIdForPage] Method 3b failed:`,
              m3bError.response?.data || m3bError.message
            );
          }
          ...
          try {
            ...
            console.log(
              `[getInstagramAccountIdForPage] Method 3c - Page with business Instagram:`,
              pageWithBusinessResponse.data
            );

            if (
              pageWithBusinessResponse.data.business?.owned_instagram_accounts
                ?.data &&
              pageWithBusinessResponse.data.business.owned_instagram_accounts
                .data.length > 0
            ) {
            ...
            console.log(
              `[getInstagramAccountIdForPage] Method 3c failed:`,
              m3cError.response?.data || m3cError.message
            );
          }
        }
      } catch (error: any) {
        console.log(
          `[getInstagramAccountIdForPage] Method 3 failed:`,
          error.response?.data || error.message
        );
      }
    }
    ...
    if (pageResponse?.data?.instagram_business_account) {
      ...
      console.log(
        `[getInstagramAccountIdForPage] Found Instagram account ID: ${instagramAccountId}`
      );
      ...
      try {
        ...
        console.log(
          `[getInstagramAccountIdForPage] Instagram account details:`,
          accountDetailsResponse.data
        );
        instagramUsername = accountDetailsResponse.data.username;
        accountType =
        ...
        // If account_type field is not available, try without it
        console.log(
          `[getInstagramAccountIdForPage] account_type not available, trying username only:`,
          accountError.response?.data || accountError.message
        );
        const usernameResponse = await axios.get<{
          username: string;
      ...
      // Get page name if not already available
      let pageName: string | undefined = pageResponse.data.name;
      if (!pageName) {
        try {
          const pageNameResponse = await axios.get<{
            name: string;
    ...
    console.error(
      "[getInstagramAccountIdForPage] Error:",
      error.response?.data || error.message
    );
    throw new Error(
      `Failed to get Instagram account: ${error.response?.data?.error?.message || error.message}`
    );
  }
}
